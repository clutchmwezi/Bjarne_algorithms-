...
int main()
{
	...
	else
	{
		...
		// Number of the bits per pixel
		fseek(image_file, 2, SEEK_CUR);
		
		fread(&bpp, 2, 1,  image_file);
		
		// Setting offseting to start
	...
	for (i = 0; i < 256; i++)
	{
		if (hist[i] != 0)
		{
			...
			// nodes as NULL pointer 
			pix_freq[j].left = NULL;
			pix_freq[j].right = NULL;
			
			
			// initiating the code 
			// word as end of the line 
			pix_freq[j].code[0] = '\0';
			j++;
		}
	}
    ...
    while (n < nodes - 1)
    {
    	...
    	// Appeding to the pix_freq Array
    	...
    	// New positon of the combined node
	...
	//Writing the huffman encode 
	// Image into a text file 
	FILE* imagehuff = fopen("encoded _image.text", "wb");
	for (i = 0; i < height; i++)
		for (j = 0; j < width; j++)
		{
			pix_val = image[i][j];
